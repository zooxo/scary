...
// Menu variables
static const byte _msgs[MAXMENU + 1][4] = { // Menu display characters
  {_7LETTERE, 0x33, _7LETTERP, _7LETTERr},               // exp_sqrt_pow_1/x
  {_7LETTERL, _7LETTERA, _7LETTERP, _7LETTERr},          // ln_annu_r2p_p2r
  {0x31, _7LETTERG, 0x5d, _7LETTERL},                    // gamma_gauss_stat_LR
  {_7LETTERt, _7LETTERS, _7LETTERc, _7LETTERt},          // _sin_cos_tan
  {_7LETTERt | _7FULLSTOP, _7LETTERS, _7LETTERc, _7LETTERt}, // _asin_acos_atan
  {_7LETTERh, _7LETTERS, _7LETTERc, _7LETTERt},          // _sinh_cosh_tanh
  {_7LETTERh | _7FULLSTOP, _7LETTERS, _7LETTERc, _7LETTERt}, // _asinh_acosh_atanh
  {_7LETTERE, _7LETTERF, _7LETTERG, _7LETTERH},          // E_F_G_H user menu
  {_7LETTERo, _7LETTERb, _7LETTERc, _7LETTERd},          // a_b_c_d user menu
};
...
void loop() {
  ...
  if (isrec && key != NULL) { // ### Record keys and write to EEPPROM
    ...
    const char *ps[11];
    ps[0] = ";,?(+2/";     // SINH: EXP ENTER INV CHS + 2 /
    ps[1] = ";,?+2/";      // COSH: EXP ENTER INV + 2 /
    ps[2] = ";,,?(+#,?+/"; // TANH: EXP ENTER ENTER INV CHS + SWAP ENTER INV + /
    ps[3] = ",,*1+>+<";    // ASINH: ENTER ENTER * 1 + SQRT + LN
    ps[4] = ",,*1->+<";    // ACOSH: ENTER ENTER * 1 - SQRT + LN
    ps[5] = ",,1+#(1+/><"; // ATANH: ENTER ENTER 1 + SWAP CHS 1 + / SQRT LN
    ps[6] = ",,,**.07*(#1.6*(+;1+?#,*(2/;.3989*";
    // GAUSS-CDF: ENTER ENTER ENTER * * .07 * CHS SWAP 1.6 * CHS + EXP 1 + INV
    // SWAP
    // GAUSS-PDF: ENTER * CHS 2 / EXP 0.3989423 *
    ps[7] = "#,,1+#&#=?(1+&&&/";
    // ANNU: SWAP ENTER ENTER 1 + SWAP ROT SWAP PWR INV CHS 1 + ROT ROT ROT /
    ps[8] = ",&&#&,&#/n&&*&&*+>";
    // R2P: ENTER ROT ROT SWAP ROT ENTER ROT SWAP / ATAN ROT ROT * ROT ROT * + SQRT
    ps[9] = ",&&#&,&#&j*&i*&&&";
    // P2R: ENTER ROT ROT SWAP ROT ENTER ROT SWAP ROT COS * ROT SIN * ROT ROT ROT
    ps[10] = "1+,,?;,?(+2/*<*2/#(+&&<#.5-*+.9189+;";
    // GAMMA: 1 + ENTER ENTER INV EXP ENTER INV CHS + 2 / * LN * 2 /
    // SWAP CHS + ROT ROT LN SWAP .5 - * + .9189385 + EXP
    // Higher precision:
    //ps[10] = "1+,,?;,?(+2/*#6=810*?+<*2/#(+&&<#.5-*+.9189358+;";
    // GAMMA: 1 + ENTER ENTER INV EXP ENTER INV CHS + 2 / * SWAP 6 PWR 810 * INV + LN * 2 /
    // SWAP CHS + ROT ROT LN SWAP .5 - * + .9189385 + EXP

    if (ps[recslot][recptr] == NULL) isplaystring = false;
    else key = ps[recslot][recptr++];
  }
